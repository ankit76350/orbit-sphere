 ...
 */
export default function EndpointTag({ id, pathParams, query, showPath = true }) {
  ...
  const label = (
    <>
      ...
      {showPath && <span className="endpoint-tag-path">{qs ? `${path}?${qs}` : path}</span>}
    </>
  )

  if (!last) {
    return (
      <span className="endpoint-tag" title="Not called yet — nothing to show">{label}</span>
    )
  }
  ...
  return (
    <button
      ...
      title={`Show the last ${endpoint.method} ${path} — ${last.status}`}
    >
      {label}
    </button>
  )
