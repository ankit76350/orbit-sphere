...
const peopleChecks = [
  ...
  ['a sub-department row opens that unit on this same page',
    orgDetailScreen.includes("navigate(detailPath('school', 'people', 'departments'")
      && orgDetailScreen.includes('data-opens')],
  ...
  ['and editing a row stops the click, or it would navigate away first',
    orgDetailScreen.includes('event.stopPropagation()\n                          openEditor(one.departmentDocsId)')],
...
const checks = [
  ...
  ['a phone and an email each identify one person, and the overrule is dated',
    staffEntry.includes('STAFF_PHONE_TAKEN') && staffEntry.includes('STAFF_EMAIL_TAKEN')
      && staffEntry.includes('overruled on 2026-09-15')],
  ['both are checked AFTER normalising, or the check is a lie',
    staffEntry.includes('Checked AFTER normalising')],
  ['both indexes are PARTIAL, and the reason is measured rather than asserted',
    staffEntry.includes('partialFilterExpression')
      && staffEntry.includes('74 schools already hold more than one person')],
  ['the service check is named as the enforcement, not the index',
    staffEntry.includes('service check is the enforcement')],
  ['the number is shaped {PREFIX}/{YYYY}/{MM}/{000001}',
