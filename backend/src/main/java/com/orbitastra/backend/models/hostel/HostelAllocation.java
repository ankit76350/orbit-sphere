...
import com.orbitastra.backend.models.hostel.enums.HostelAllocationStatus;
 ...
 *
 * <p>
 * This is the record that connects a child to a bed, and it is also what causes
 * them
 * to be charged. It follows exactly the shape TransportAllocation set: the
 * amounts are
 * copied on at allocation time and never rewritten, and a fee head says where
 * the charge
 * lands.
 *
 * <p>
 * Three separate charges live here because a boarding school raises three:
 *
 * <ul>
 * <li>{@code monthlyHostelFee} for the bed, billed under
 * FeeCategory.HOSTEL.</li>
 * <li>{@code monthlyMessFee} for the food, billed under FeeCategory.MESS. Mess
 * charges
 * ride on this record rather than on a separate subscription because at a
 * boarding
 * school everybody who has a bed eats.</li>
 * <li>{@code securityDepositAmount} taken once, billed under
 * FeeCategory.DEPOSIT, and
 * given back at the end less anything owed. It is money held rather than money
 * earned,
 * which is why it has its own category and its own refund date.</li>
 * </ul>
 *
 * <p>
 * A student may hold only one ACTIVE allocation per year, which the unique
 * index
 * enforces. Moving room means ending one and starting another, so the change
 * has a date
 * on it and the old arrangement is still readable.
 *
 * <p>
 * A child away for a weekend stays ACTIVE. Going home for two nights is a leave
 * request, not a change of residence, and treating it as one would take them
 * off the
 ...
 *
 * <p>
 * The service checks that the bed is AVAILABLE and in a building whose
 * HostelType
 * suits the child, that the bed is set to OCCUPIED in the same step, that the
 * fee heads
