...
import com.orbitastra.backend.models.people.staff.enums.CredentialVerificationStatus;
import com.orbitastra.backend.models.people.staff.enums.StaffCredentialType;
 ...
 *
 * <p>
 * Credential numbers may be encrypted and searched using an optional keyed
 * lookup hash. The evidence document remains in the document-storage module.
 */
@Document(collection = "staff_credentials")
@CompoundIndexes({
                @CompoundIndex(name = "school_staff_credential_lookup_uniq", def = "{'schoolId': 1, 'staffDocsId': 1, 'credentialType': 1, 'credentialNumberLookupHash': 1}", unique = true, partialFilter = "{'credentialNumberLookupHash': {'$type': 'string'}}"),
                @CompoundIndex(name = "school_credential_status_expiry_idx", def = "{'schoolId': 1, 'verificationStatus': 1, 'validUntil': 1}")
})
@Data
...
// StaffCredential stores qualifications and certificates belonging to a staff member.
        // For example, one teacher may have:
                // Bachelor of Education certificate
                // Teaching licence
                // Police/background verification
                // First-aid certificate
                // Child-safety training certificate
        ...
        @NotNull
        @Builder.Default
        private CredentialVerificationStatus verificationStatus = CredentialVerificationStatus.UNVERIFIED;
