...
The `models/undone` tree was deleted on 2026-08-21. Two sets of sketches in it belonged to this
package, and what they held is recorded here so nothing is lost:

- **Fee dunning** — `FeeReminderLog` with a `ReminderChannel` enum. The live copies under
  `finance/dunning` were removed on 2026-08-12 by decision. Chasing an unpaid bill is a message,
  and messages live here.
- **Birthday greetings** — `BirthdayGreeting`, holding `birthdayPersonDocsId`, `senderDocsId`,
  `message`, `imageUrl`, `anonymous`, `autoGenerated` and `sentAt`. Those last two fields are
  the giveaway: it is a notification with a nice subject line, not its own module. Its
  `anonymous` boolean should be read against
  [`FeedbackAnonymityMode`](../feedback/enums/FeedbackAnonymityMode.java) before it is trusted —
  a boolean cannot hold that promise, which is the whole argument in
  [`feedback/README.md`](../feedback/README.md).
- **A wider communication surface** also existed and was deleted: `ConversationThread` and
  `ConversationMessage` (parent-teacher messaging), `PtmEvent` and `PtmBooking` (parent-teacher
  meeting slots), `DiaryEntry`, `Announcement`, `CommunicationCampaign`, `MessageTemplate`,
  `CommunicationPreference`, `DeliveryEndpoint`, `CommunicationLog` and `Notification`. The
  useful ideas in that set were: **one endpoint per channel per person** with the encrypted /
  lookup-hash pair, **preferences per purpose per channel** with quiet hours and a locale, and
  **a delivery log separate from the message** so one notification sent three ways is three
  delivery rows. Design them properly when this package is built; the list above is the input,
  not the answer.
