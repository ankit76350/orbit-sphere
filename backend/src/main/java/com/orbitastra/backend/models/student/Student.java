 ...
 *
 * <p>
 * Academic-year placement is stored in StudentAcademicRecord. Health, hostel,
 * transport, attendance, fees, and wallet data belong to their own modules and
 * are not duplicated in this profile.
 */
@Document(collection = "students")
@CompoundIndexes({
                @CompoundIndex(name = "school_admission_no_uniq", def = "{'schoolId': 1, 'admissionNo': 1}", unique = true),
                @CompoundIndex(name = "school_admission_application_uniq", def = "{'schoolId': 1, 'admissionApplicationDocsId': 1}", unique = true, partialFilter = "{'admissionApplicationDocsId': {'$type': 'string'}}"),
                @CompoundIndex(name = "school_student_status_name_idx", def = "{'schoolId': 1, 'status': 1, 'fullName': 1}"),
                @CompoundIndex(name = "school_guardian_students_idx", def = "{'schoolId': 1, 'guardians.guardianDocsId': 1}"),
                @CompoundIndex(name = "school_current_academic_record_uniq", def = "{'schoolId': 1, 'currentAcademicRecordDocsId': 1}", unique = true, partialFilter = "{'currentAcademicRecordDocsId': {'$type': 'string'}}")
})
@Data
