...
/**
 * Every IANA time-zone id, as a closed set.
 *
 * <p><b>Why an enum rather than a validated String.</b> A zone decides which calendar day a
 * stored instant falls on, so a typo in one is not cosmetic — it silently moves a school's dates.
 * As a String the field could hold anything and only failed when something tried to use it; as an
 * enum an unknown value is refused at the edge of the system, once, with a message naming it.
 *
 * <p><b>THE WIRE VALUE IS THE IANA ID, NOT THE CONSTANT NAME.</b> ASIA_KOLKATA is how Java has to
 * spell it — a constant cannot contain a slash — but every request, response and stored document
 * says "Asia/Kolkata". That is what {@link #getId()} with {@code @JsonValue} and {@link #fromId}
 * with {@code @JsonCreator} are for, and it is not optional: thousands of documents already hold
 * the IANA form, and the frontend, the Postman collection and every README example speak it too.
 * An enum serialising as its constant name would have broken all of them at once.
 *
 * <p>Mongo needs telling separately — Spring Data stores an enum as {@code name()} unless a
 * converter says otherwise. See {@code config/EnumCodeConfig}.
 *
 * <p><b>Generated from {@code ZoneId.getAvailableZoneIds()}</b> rather than typed, so the list is
 * whatever the JDK actually accepts and no constant can name a zone {@link ZoneId#of} would
 * reject. {@link #toZoneId()} is the bridge back.
 */
public enum SchoolTimeZone {
    ...
    EIRE("Eire"),
    ETC_GMT("Etc/GMT"),
    ETC_GMTPLUS_0("Etc/GMT+0"),
    ETC_GMTPLUS_1("Etc/GMT+1"),
    ETC_GMTPLUS_10("Etc/GMT+10"),
    ETC_GMTPLUS_11("Etc/GMT+11"),
    ETC_GMTPLUS_12("Etc/GMT+12"),
    ETC_GMTPLUS_2("Etc/GMT+2"),
    ETC_GMTPLUS_3("Etc/GMT+3"),
    ETC_GMTPLUS_4("Etc/GMT+4"),
    ETC_GMTPLUS_5("Etc/GMT+5"),
    ETC_GMTPLUS_6("Etc/GMT+6"),
    ETC_GMTPLUS_7("Etc/GMT+7"),
    ETC_GMTPLUS_8("Etc/GMT+8"),
    ETC_GMTPLUS_9("Etc/GMT+9"),
    ETC_GMT_0("Etc/GMT-0"),
    ETC_GMT_1("Etc/GMT-1"),
    ...
    /** How many zones this enum covers. */
    public static final int TOTAL = 603;

    private static final Map<String, SchoolTimeZone> BY_ID = Stream.of(values())
            .collect(Collectors.toUnmodifiableMap(SchoolTimeZone::getId, Function.identity()));

    private final String id;

    SchoolTimeZone(String id) {
        this.id = id;
    }

    /** The IANA id, which is what goes on the wire and into the database. */
    @JsonValue
    public String getId() {
        return id;
    }
    ...
    /**
     * The zone named by an IANA id.
     *
     * <p>Exact and case-sensitive, because IANA ids are: {@code Asia/Kolkata} is a zone and
     * {@code asia/kolkata} is not, and quietly accepting the second would let two spellings of
     * one zone into the data.
     *
     * @throws IllegalArgumentException naming the offending value, which the request handler
     *                                 turns into a 400 naming the field
     */
    @JsonCreator
    public static SchoolTimeZone fromId(String id) {
        SchoolTimeZone found = id == null ? null : BY_ID.get(id.trim());
        if (found == null) {
            throw new IllegalArgumentException(
                    "'" + id + "' is not an IANA time-zone id. Use one of the " + TOTAL
                            + " zones, such as Asia/Kolkata.");
        }
        return found;
    }

    /** The JDK zone, for date arithmetic. Never throws: the constant came from this list. */
