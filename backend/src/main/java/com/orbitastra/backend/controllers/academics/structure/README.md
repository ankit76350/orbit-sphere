...
school_year_term_name_uniq    {schoolId, academicYear, name}        unique
```
...
| `TERM_NAME_TAKEN` | 409 | that year already has a term with that `name`, case-insensitively — added 2026-09-12, and retired terms hold theirs |
...
| `TERM_WEIGHT_MIXED` | 409 | one active term carries a weight and another does not — added with #1. Distinct from the above: a *shortfall* is transient and only reported, a *mixture* computes to nothing and is refused |
| `TERM_WEIGHTS_EXCEED_100` | 409 | this term's weight would take the year's active weights past 100% — added with #1. The ceiling only: a *shortfall* stays a warning, because a year being entered sits under 100 until the last term arrives |
...
- **Which makes the choice of fallback the decision that matters.** `sequence` is unique within a year — #1 enforces it and `school_year_term_sequence_uniq` declares it — so every sort ends in a total order and paging cannot put one row on two pages while another appears on none. A term `name` is unique too as of 2026-09-12 and *could* now have served, but `sequence` is what a year is actually ordered by: a fallback that reshuffled the page whenever two terms were renamed would be a worse one for being equally valid. **A hand-written `withStableOrder` was added here first and deleted the same day**, once a mutation removing it changed nothing — the shared helper already did it.
