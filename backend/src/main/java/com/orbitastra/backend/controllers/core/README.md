...
**Nineteen of 28 endpoints are built — #1 to #9 and #18 to #27; Phases 1 to 5 and the gates from
Phase 6**, plus the two `DELETE`s that pair with the calendar endpoints. This is the complete inventory of every `POST`,
...
They live in [`AcademicYearService`](../../services/core/AcademicYearService.java) with the rest
of the year's endpoints; one resource, one service.

| # | What it gates | Idempotent | Reason | Audited |
|---|---|---|---|---|
| 24 | admissions may write to this year | yes | — | no |
| 25 | admissions may not | yes | — | no |
| 26 | results are frozen | yes | — | **yes** |
| 27 | results are editable again | **no — 409** | **required** | **yes, refusals too** |

### Enrollment is a switch; results are not

#24 and #25 flip freely and neither direction is destructive: a year already open comes back
`200` saying so. A `409` there would only teach callers to read first and then race.

#25 **does not touch students already enrolled.** It is a gate on new writes, not a withdrawal.

### #27 is the one to be careful with

Locking results is routine; unlocking means somebody can change a mark a parent has already seen.
It differs from the other three in three ways, each for its own reason:

**A reason is required**, 10 to 500 characters, and it goes on the audit row. It is the only part
of that row a person can read six months later and understand — everything else says what
happened, and this says why. The minimum length will not stop somebody typing ten characters of
nothing; it is there so the default path is a real sentence.

**Every call is audited, refusals included** — and that needed a specific piece of care. A
refusal ends in a thrown exception, which rolls the transaction back and would take the evidence
with it. So [`AuditTrail`](../../common/audit/AuditTrail.java) has two methods: `record` joins the
caller's transaction, so a change and its evidence commit together or not at all; and
`recordOutsideTransaction` runs `REQUIRES_NEW`, so a refusal survives the rollback that refused
it. Using the wrong one would silently lose exactly the events somebody will one day come looking
for.

**Unlocking a year that is not locked is a `409`**, not a quiet success. A no-op logged as a
successful unlock would put a row in the trail claiming results were opened when they never were.

**What is deliberately not audited:** a request that fails validation — no reason, or a reason
under ten characters. It never reaches the service, nothing was unlocked or even attempted, and
filling the trail with client bugs would bury the attempts that matter.

### `action` names the operation, not the outcome

Both a successful and a refused unlock are written as `RESULTS_UNLOCK`, with `outcome` carrying
the result. One query on the action returns every attempt. Two actions — `RESULTS_UNLOCK` and
`RESULTS_UNLOCK_FAILED` — would mean a search for unlocks quietly missing the failures.

#26 is audited too, though it is the safe direction. A lock on its own is uninteresting; a lock
and an unlock **together** are what say how long results were open and who opened them. Both
carry `retainLongTerm: true`, because how long a year's marks were editable is a question about a
child's academic record and is asked years later or not at all.

### Two gaps these endpoints announce rather than hide
...
`nextStep`, the same way #3 announces its subscription gap. The audit rows are written now
regardless, because a trail that starts the day permissions arrive can say nothing about the
months before it.

**The actor is `ANONYMOUS` on every row.** There is no authentication, so nothing knows who is
acting, and a guess would be worse than the gap: an audit trail naming the wrong person is
evidence against the wrong person. The address and user agent are recorded as the only real
evidence available, and both are a lead rather than a proof. When sessions exist, one method in
`AuditTrail` changes.

### `audit_events` has no repository, on purpose

A `MongoRepository` cannot be insert-only — it arrives with `save`, `delete` and `deleteAll`, and
`save` silently becomes an update when the id is set. `AuditEvent`'s own javadoc asks for an
insert-only interface, so `AuditTrail` calls `MongoTemplate.insert` instead, which fails on a
duplicate id rather than overwriting. History cannot be edited even by mistake.

There is **no read endpoint** for the trail yet; reads are a separate pass. Until then:
...
| authorization for lock/unlock and enrollment | 24–27 — **not built, see below** |
