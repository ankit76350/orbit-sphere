...
**The numbers are #1 to #29 and G1 to G11, and they are not renumbered.** They are quoted from the
...
**What the marker in the `#` column means**, the same four words [`controllers/plans`](../plans/README.md)
uses:

| Marker | Meaning |
|---|---|
| **built** | It exists and answers. 32 of them. |
| **deferred** | Planned, not started, and not next. |
| **optional** | A convenience nobody is waiting on. |
| **not being built** | Decided against, struck through, and the reason is in its own section. |

Anything unmarked does not exist yet, and a request to it returns a 404. Every marker in the table
is repeated on that endpoint's own section below, so the two cannot drift.
...
|---|---|---|---|
| <a id="t1"></a>#1 — **built** | [`POST /platform/schools`](#e1) | Create a tenant. This is the first thing that happens for a new customer, and it is the only document in the system with no `schoolId` above it.  | [`schools`](../../models/core/School.java) |
| <a id="t2"></a>#2 — **built** | [`POST /platform/schools/{id}/complete-provisioning`](#e2) | Finish the setup #1 leaves undone: create every missing number sequence and the starting roles. Safe to run twice — it only fills in what is missing.  | [`schools`](../../models/core/School.java), [`number_sequences`](../../models/institution/NumberSequence.java), [`roles`](../../models/identity/Role.java) |
| <a id="t3"></a>#3 — **built** | [`POST /platform/schools/{id}/activate`](#e3) | Take the school live. Refuses unless #2 has actually run, because a school with no `SCHOOL_ADMIN` role or missing sequences fails on first use rather than at activation.  | [`schools`](../../models/core/School.java), [`roles`](../../models/identity/Role.java), [`number_sequences`](../../models/institution/NumberSequence.java), [`school_subscriptions`](../../models/plans/SchoolSubscription.java) |
| <a id="t4"></a>#4 — **built** | [`POST /platform/schools/{id}/suspend`](#e4) | Block a school and record why. Only an `ACTIVE` school can be suspended.  | [`schools`](../../models/core/School.java) |
| <a id="t5"></a>#5 — **built** | [`POST /platform/schools/{id}/reactivate`](#e5) | Put a suspended school back. `suspendedAt` and the old reason are kept on purpose, as the record of the last suspension.  | [`schools`](../../models/core/School.java) |
| <a id="t10"></a>#10 — **built** | [`PATCH /platform/schools/{id}/subdomain`](#e10) | Change the label a school answers to. On the platform surface because this is the key that resolves every request to the tenant, not a profile detail.  | [`schools`](../../models/core/School.java) |
| <a id="t12"></a>#12 — **deferred** | [`POST /platform/schools/{id}/rotate-encryption-key`](#e12) | **Deferred.** Point the school at a new key. Deferred because nothing is encrypted yet, so there is no key to rotate. | [`schools`](../../models/core/School.java) |
| <a id="t13"></a>#13 — **deferred** | [`POST /platform/schools/{id}/offboard`](#e13) | **Deferred.** Start winding a school down. Deferred until offboarding is actually wanted. | [`schools`](../../models/core/School.java) |
| <a id="t14"></a>#14 — **deferred** | [`POST /platform/schools/{id}/close`](#e14) | **Deferred.** Close a school that has finished offboarding. | [`schools`](../../models/core/School.java) |
| <a id="t15"></a>#15 — **deferred** | [`POST /platform/schools/{id}/request-deletion`](#e15) | **Deferred.** Ask for the school's data to be erased, starting a waiting period. | [`schools`](../../models/core/School.java) |
| <a id="t16"></a>#16 — **deferred** | [`POST /platform/schools/{id}/cancel-deletion`](#e16) | **Deferred.** Change our mind during the waiting period. Not in the model's README — proposed here. | [`schools`](../../models/core/School.java) |
| <a id="t17"></a>#17 — **deferred** | [`POST /platform/schools/{id}/confirm-deletion`](#e17) | **Deferred.** Actually erase the data once the waiting period is over. | [`schools`](../../models/core/School.java) |
...
| # | Method and endpoint | What this API is for | Collections it touches |
|---|---|---|---|
| <a id="t6"></a>#6 — **built** | [`PATCH /schools/current/profile`](#e6) | The school edits its own name and contact details. Partial: a missing field is left alone, an empty string clears it, an empty body is a 400.  | [`schools`](../../models/core/School.java) |
| <a id="t7"></a>#7 — **built** | [`PUT /schools/current/address`](#e7) | Replace the whole postal address. All-or-nothing on purpose: a patched address can name a city in the wrong state.  | [`schools`](../../models/core/School.java) |
| <a id="t8"></a>#8 — **built** | [`PATCH /schools/current/localization`](#e8) | Set language and time zone. The time zone reinterprets which calendar date every existing attendance record and holiday falls on, so it needs confirming and is refused mid-year.  | [`schools`](../../models/core/School.java), [`academic_years`](../../models/core/AcademicYear.java) |
| <a id="t9"></a>#9 — **built** | [`PUT /schools/current/logo`](#e9) | Replace the logo, or remove it when the URL is blank. https and an allow-listed host only.  | [`schools`](../../models/core/School.java) |
| <a id="t11"></a>~~#11~~ **not being built** | [~~PATCH /platform/schools/{id}/account-holder~~](#e11) | **Dropped.** Dropped on 2026-08-31 and folded into #6. The account holder's name is an ordinary profile field, and a second endpoint for one string is a second thing to keep in step. | [`schools`](../../models/core/School.java) |
...
| # | Method and endpoint | What this API is for | Collections it touches |
|---|---|---|---|
| <a id="t18"></a>#18 — **built** | [`POST /schools/current/academic-years`](#e18) | Create a year with an empty calendar. **The name can never change** — every other collection stores it as a string, so a rename would orphan them all silently.  | [`schools`](../../models/core/School.java), [`academic_years`](../../models/core/AcademicYear.java) |
| <a id="t19"></a>#19 — **built** | [`PATCH /schools/current/academic-years/{name}/dates`](#e19) | Move the start or end date. Refused when a closed day would end up outside the new range, because a holiday stored outside its year is invisible to every query that asks about it.  | [`academic_years`](../../models/core/AcademicYear.java) |
| <a id="t20"></a>#20 — **built** | [`PUT /schools/current/academic-years/{name}/holidays`](#e20) | Replace the whole calendar in one go. What a school does when it has the year's holiday list from the board and wants it in as one action.  | [`academic_years`](../../models/core/AcademicYear.java) |
| <a id="t21"></a>#21 — **built** | [`POST /schools/current/academic-years/{name}/holidays`](#e21) | Add one closed day, or add a second reason to a day that is already closed. A Sunday that is also Diwali is one day with two reasons.  | [`academic_years`](../../models/core/AcademicYear.java) |
| <a id="t22"></a>#22 — **built** | [`PATCH /schools/current/academic-years/{name}/holidays/{date}?type=`](#e22) | Rename a reason, change its description, or change its type. The date itself cannot move — remove it and add it back instead.  | [`academic_years`](../../models/core/AcademicYear.java) |
| <a id="td1"></a>D1 — **built** | [`DELETE /schools/current/academic-years/{name}/holidays/{date}?type=`](#ed1) | Remove one reason from a day, or the whole day when no type is given. A day that loses its last reason becomes a working day again.  | [`academic_years`](../../models/core/AcademicYear.java) |
| <a id="t23"></a>#23 — **built** | [`POST /schools/current/academic-years/{name}/holidays/generate-weekly-off`](#e23) | Turn "we are closed on Sundays" into the ~52 dated entries the model requires. Not a convenience: every closure has to be a real date, so without this somebody types 52 of them.  | [`academic_years`](../../models/core/AcademicYear.java) |
| <a id="td2"></a>D2 — **built** | [`DELETE /schools/current/academic-years/{name}/holidays?type=`](#ed2) | Clear every closure of one type. `type` is required precisely because forgetting it must not wipe a whole calendar.  | [`academic_years`](../../models/core/AcademicYear.java) |
| <a id="t24"></a>#24 — **built** | [`POST /schools/current/academic-years/{name}/enrollment/enable`](#e24) | Open the year to new enrollments. Idempotent — already open comes back 200 saying so.  | [`academic_years`](../../models/core/AcademicYear.java) |
| <a id="t25"></a>#25 — **built** | [`POST /schools/current/academic-years/{name}/enrollment/disable`](#e25) | Close the year to new enrollments. A gate on new writes only — students already enrolled are untouched.  | [`academic_years`](../../models/core/AcademicYear.java) |
| <a id="t26"></a>#26 — **built** | [`POST /schools/current/academic-years/{name}/results/lock`](#e26) | Lock results against further change. What happens when marks are published.  | [`academic_years`](../../models/core/AcademicYear.java) |
| <a id="t27"></a>#27 — **built** | [`POST /schools/current/academic-years/{name}/results/unlock`](#e27) | Unlock results so they can be corrected. **Records nothing about who unlocked, or why** — see the debt noted below.  | [`academic_years`](../../models/core/AcademicYear.java) |
| <a id="t29"></a>#29 — **built** | [`POST /schools/current/academic-years/{name}/end`](#e29) | End the year today: stop it running and close its dates on today, in the school's own timezone. **Refuses to extend** a year that already finished, and refuses one that has not started. Allows a year cut very short, which [#19](#t19) rejects as implausible. Re-ending is a 200 saying nothing changed.  | [`academic_years`](../../models/core/AcademicYear.java) |
| <a id="t28"></a>#28 — **optional** | [`POST /schools/current/academic-years/{name}/clone`](#e28) | **Optional.** Copy last year's calendar into a new year, so a school does not re-enter it. Convenience only — #18 plus #20 already do it. | [`academic_years`](../../models/core/AcademicYear.java) |
...
| # | Method and endpoint | What this API is for | Collections it touches |
|---|---|---|---|
| <a id="tg1"></a>G1 — **built** | [`GET /platform/schools`](#eg1) | The operator's school list: filter, search, sort, page. A bare call gives the newest twenty, which is what somebody opening the console usually wants.  | [`schools`](../../models/core/School.java) |
| <a id="tg2"></a>G2 — **built** | [`GET /platform/schools/{id}`](#eg2) | One school in full for the operator, including the lifecycle fields the school itself never sees. Returns a school at any status — closed and deleted included.  | [`schools`](../../models/core/School.java) |
| <a id="tg3"></a>~~G3~~ **not being built** | [~~GET /platform/schools/subdomain-available?value=~~](#eg3) | **Dropped.** Dropped on 2026-08-31, having been built the same day. #1 and #10 already answer it with the same codes, so a signup form submits once instead of asking per keystroke. | [`schools`](../../models/core/School.java) |
...
| # | Method and endpoint | What this API is for | Collections it touches |
|---|---|---|---|
| <a id="tg4"></a>G4 — **built** | [`GET /schools/current`](#eg4) | The school reading its own details — the read behind #6 to #9. Returns the identical record those four return, so one screen loads and saves with one shape.  | [`schools`](../../models/core/School.java) |
| <a id="tg5"></a>G5 — **built** | [`GET /schools/current/academic-years`](#eg5) | Every year the school has, newest first. Sorted on `startDate`, not `createdAt` — "newest" means furthest along the calendar, not typed most recently.  | [`academic_years`](../../models/core/AcademicYear.java) |
| <a id="tg6"></a>G6 — **built** | [`GET /schools/current/academic-years/current`](#eg6) | The year today falls in, or a 404 that says which kind of nothing. Worked out from the dates, never stored — a `current` flag would be a second source that can disagree.  | [`academic_years`](../../models/core/AcademicYear.java) |
| <a id="tg7"></a>G7 — **built** | [`GET /schools/current/academic-years/{name}`](#eg7) | One year by name. Keyed on the name because that is what the whole system means when it says "which year", and the lookup is by school and name, so another school's year is a 404.  | [`academic_years`](../../models/core/AcademicYear.java) |
| <a id="tg8"></a>G8 — **built** | [`GET /schools/current/academic-years/{name}/holidays`](#eg8) | The whole calendar, sorted by date, with both counts. `closedDayCount` is days; `eventCount` is reasons, and it is larger whenever a weekly off lands on a festival.  | [`academic_years`](../../models/core/AcademicYear.java) |
| <a id="tg9"></a>G9 — **built** | [`GET /schools/current/academic-years/{name}/holidays/{date}`](#eg9) | **Is the school closed that day, and why.** The question attendance, timetables, transport and fee due dates all ask. An open day is a 200 with `closed: false`, never a 404.  | [`academic_years`](../../models/core/AcademicYear.java) |
| <a id="tg10"></a>G10 — **built** | [`GET /schools/current/academic-years/{name}/working-days?from=&to=`](#eg10) | Which days in a range are working days, and how many. G9 in bulk — attendance percentages and fee proration need the whole range, not two hundred calls.  | [`academic_years`](../../models/core/AcademicYear.java) |
| <a id="tg11"></a>G11 — **optional** | [`GET /schools/current/academic-years/{name}/holidays/export?format=csv`](#eg11) | **Optional.** The calendar as a file a school can hand to somebody. Optional — G8 already returns it as JSON. | [`academic_years`](../../models/core/AcademicYear.java) |
...
| `isThisYearRunning` | Boolean, required | **`true`** at create — every new year is marked as the one the school is using. **Null on every document written before 2026-09-09**, which reads as not running. `false` from [#29](#e29); nothing else writes it. It says the school has *switched over* to a year the calendar says has begun, which the dates cannot say on their own. Read only **alongside** the dates, never alone: one left true after its year ended would keep a finished year live. **Because it defaults to true and nothing enforces one-per-school, several years can read true at once today** — so it currently narrows nothing for a freshly created year, and wants a partial unique index on `{schoolId, isThisYearRunning}` filtered to true. `AcademicYearResponse` argues against storing this at all; see its note on `current`. |
...
<a id="e1"></a>
**[#1](#t1) · `POST /platform/schools`** — built
...
<a id="e2"></a>
**[#2](#t2) · `POST /platform/schools/{id}/complete-provisioning`** — built
...
<a id="e3"></a>
**[#3](#t3) · `POST /platform/schools/{id}/activate`** — built
...
<a id="e4"></a>
**[#4](#t4) · `POST /platform/schools/{id}/suspend`** — built
...
<a id="e5"></a>
**[#5](#t5) · `POST /platform/schools/{id}/reactivate`** — built
...
<a id="e10"></a>
**[#10](#t10) · `PATCH /platform/schools/{id}/subdomain`** — built
...
<a id="e6"></a>
**[#6](#t6) · `PATCH /schools/current/profile`** — built
...
<a id="e7"></a>
**[#7](#t7) · `PUT /schools/current/address`** — built
...
<a id="e8"></a>
**[#8](#t8) · `PATCH /schools/current/localization`** — built
...
<a id="e9"></a>
**[#9](#t9) · `PUT /schools/current/logo`** — built
...
<a id="e11"></a>
**[#11](#t11) · `PATCH /platform/schools/{id}/account-holder`** — not being built
...
<a id="e18"></a>
**[#18](#t18) · `POST /schools/current/academic-years`** — built
...
<a id="e19"></a>
**[#19](#t19) · `PATCH /schools/current/academic-years/{name}/dates`** — built
...
<a id="e20"></a>
**[#20](#t20) · `PUT /schools/current/academic-years/{name}/holidays`** — built
...
<a id="e21"></a>
**[#21](#t21) · `POST /schools/current/academic-years/{name}/holidays`** — built
...
<a id="e22"></a>
**[#22](#t22) · `PATCH /schools/current/academic-years/{name}/holidays/{date}?type=`** — built
...
<a id="ed1"></a>
**[D1](#td1) · `DELETE /schools/current/academic-years/{name}/holidays/{date}?type=`** — built
...
<a id="e23"></a>
**[#23](#t23) · `POST /schools/current/academic-years/{name}/holidays/generate-weekly-off`** — built
...
<a id="ed2"></a>
**[D2](#td2) · `DELETE /schools/current/academic-years/{name}/holidays?type=`** — built
...
<a id="e24"></a>
**[#24](#t24) · `POST /schools/current/academic-years/{name}/enrollment/enable`** — built
...
<a id="e25"></a>
**[#25](#t25) · `POST /schools/current/academic-years/{name}/enrollment/disable`** — built
...
<a id="e26"></a>
**[#26](#t26) · `POST /schools/current/academic-years/{name}/results/lock`** — built
...
<a id="e27"></a>
**[#27](#t27) · `POST /schools/current/academic-years/{name}/results/unlock`** — built
...
<a id="e29"></a>
**[#29](#t29) · `POST /schools/current/academic-years/{name}/end`** — built
...
<a id="eg1"></a>
**[G1](#tg1) · `GET /platform/schools`** — built
...
<a id="eg2"></a>
**[G2](#tg2) · `GET /platform/schools/{id}`** — built
...
<a id="eg3"></a>
**[G3](#tg3) · `GET /platform/schools/subdomain-available?value=`** — not being built
...
<a id="eg4"></a>
**[G4](#tg4) · `GET /schools/current`** — built
...
<a id="eg5"></a>
**[G5](#tg5) · `GET /schools/current/academic-years`** — built
...
<a id="eg6"></a>
**[G6](#tg6) · `GET /schools/current/academic-years/current`** — built
...
<a id="eg7"></a>
**[G7](#tg7) · `GET /schools/current/academic-years/{name}`** — built
...
<a id="eg8"></a>
**[G8](#tg8) · `GET /schools/current/academic-years/{name}/holidays`** — built
...
<a id="eg9"></a>
**[G9](#tg9) · `GET /schools/current/academic-years/{name}/holidays/{date}`** — built
...
<a id="eg10"></a>
**[G10](#tg10) · `GET /schools/current/academic-years/{name}/working-days?from=&to=`** — built
